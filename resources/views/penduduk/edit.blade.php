<x-app-layout>
    <x-slot name="header">
        <div class="py-2">
            ...
            <p class="text-forest/60 text-sm mt-1">Sesuaikan informasi profil penduduk dengan teliti demi akurasi data.</p>
        </div>
    </x-slot>

    <div class="max-w-4xl mx-auto pb-12">
        <form action="{{ route('penduduk.update', $penduduk) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white/70 backdrop-blur-sm p-8 rounded-2xl shadow-sm border border-premium-border/50">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <x-text-input id="nik" name="nik" type="text" class="mt-1 block w-full" :value="old('nik', $penduduk->nik)" required autofocus maxlength="16" minlength="16" oninput="this.value = this.value.replace(/[^0-9]/g, '').slice(0, 16)" />
                        <x-input-error class="mt-2" :messages="$errors->get('nik')" />
                    </div>

                    <div>
                        <x-input-label for="nama_lengkap" :value="__('Nama Lengkap')" />
                        <x-text-input id="nama_lengkap" name="nama_lengkap" type="text" class="mt-1 block w-full" :value="old('nama_lengkap', $penduduk->nama_lengkap)" required />
                    ...
                    <div>
                        ...
                        <select id="kelurahan_id" name="kelurahan_id" class="mt-1 block w-full border-premium-border/40 bg-white/50 focus:border-forest focus:ring-forest rounded-xl shadow-sm transition-all duration-300 text-sm font-medium text-forest" required>
                            @foreach($kelurahan as $k)
                                <option value="{{ $k->id }}" {{ old('kelurahan_id', $penduduk->kelurahan_id) == $k->id ? 'selected' : '' }}>{{ $k->nama }}</option>
                            @endforeach
                        </select>
            ...
            <div class="bg-white/70 backdrop-blur-sm p-8 rounded-2xl shadow-sm border border-premium-border/50">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <x-text-input id="jumlah_tanggungan" name="jumlah_tanggungan" type="number" class="mt-1 block w-full" :value="old('jumlah_tanggungan', $penduduk->jumlah_tanggungan)" required />
