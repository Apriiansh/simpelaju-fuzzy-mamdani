<x-app-layout>
    ...
    <div class="mb-8">
        <p class="text-forest/60 text-sm italic">Perbarui data pengguna {{ $user->name }}.</p>
    ...
    <div class="bg-white/60 backdrop-blur-md overflow-hidden shadow-sm rounded-[2.5rem] border border-premium-border/50 p-10">
        <form action="{{ route('users.update', $user) }}" method="POST" class="space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                {{-- Password --}}
                <div>
                    <label for="password" class="block text-[10px] font-black text-forest/50 uppercase tracking-[0.2em] mb-3">Password Baru</label>
                    <input type="password" name="password" id="password" minlength="8"
                        class="w-full bg-paper/30 border border-premium-border/30 rounded-xl px-6 py-4 text-forest placeholder-forest/30 focus:outline-none focus:border-forest/50 focus:bg-white transition-all duration-300">
                    <p class="mt-2 text-forest/40 text-xs">Kosongkan jika tidak ingin mengubah password</p>
                    @error('password')
                        <p class="mt-2 text-red-500 text-xs">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="role" id="role" required onchange="toggleKelurahan()"
                        ...
                        <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>Admin Camat</option>
                        <option value="operator" {{ old('role', $user->role) === 'operator' ? 'selected' : '' }}>Operator Lurah</option>
            ...
            <div id="kelurahan-container" class="{{ old('role', $user->role) === 'operator' ? '' : 'hidden' }}">
                ...
                <select name="kelurahan_id" id="kelurahan_id"
                    ...
                    @foreach($kelurahan as $k)
                        <option value="{{ $k->id }}" {{ old('kelurahan_id', $user->kelurahan_id) == $k->id ? 'selected' : '' }}>{{ $k->nama }}</option>
                    @endforeach
                </select>
                @error('kelurahan_id')
                    <p class="mt-2 text-red-500 text-xs">{{ $message }}</p>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', toggleKelurahan);
    </script>
</x-app-layout>
