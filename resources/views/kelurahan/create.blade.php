    ...
    <div class="py-12">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex items-center justify-between">
                <div>
                    <h2 class="font-serif text-3xl font-bold text-forest leading-tight tracking-tight">
                        Tambah <span class="text-amber italic">Kelurahan</span>
                    </h2>
                    <p class="mt-2 text-sm text-forest/60 font-medium">Lengkapi data wilayah administratif di bawah ini.</p>
                </div>
            ...
            <div class="bg-white/70 backdrop-blur-sm rounded-2xl shadow-sm border border-premium-border/50 overflow-hidden">
                <form action="{{ route('kelurahan.store') }}" method="POST" class="p-8 space-y-6">
                    @csrf
                    
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            <x-input-label for="nama" :value="__('Nama Kelurahan')" />
                            <x-text-input id="nama" name="nama" type="text" class="mt-1 block w-full" :value="old('nama')" required autofocus placeholder="Masukkan nama kelurahan..." />
                            <x-input-error class="mt-2" :messages="$errors->get('nama')" />
                        </div>

                        <div>
                            <x-input-label for="kode" :value="__('Kode Wilayah')" />
                            <x-text-input id="kode" name="kode" type="text" class="mt-1 block w-full" :value="old('kode')" placeholder="Contoh: 32.73.xx..." />
                        ...
                        <div>
                            ...
                            <textarea id="batas_wilayah" name="batas_wilayah" rows="4" 
                                class="mt-1 block w-full border-premium-border/40 bg-white/50 focus:border-forest focus:ring-forest rounded-xl shadow-sm transition-all duration-300 placeholder-forest/30"
                                placeholder="Jelaskan batas wilayah kelurahan...">{{ old('batas_wilayah') }}</textarea>
